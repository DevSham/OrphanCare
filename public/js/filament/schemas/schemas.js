...
(() => {
    ...
    document.addEventListener("alpine:init", () => {
        ...
        window.Alpine.data("filamentSchemaComponent", ({path: i, containerPath: e, $wire: n}) => ({
            $statePath: i,
            $get: (t, o) => n.$get(m(e, t, o)),
            $set: (t, o, l, a = !1) => n.$set(m(e, t, l), o, a),
            get $state() {
                return n.$get(i)
            }
        }));
        ...
        // Wait for Livewire to be available before setting up hook
        const setupLivewireHook = () => {
            if (typeof Livewire === 'undefined' || !Livewire.hook) {
                // Livewire not ready yet, wait a bit
                setTimeout(setupLivewireHook, 50);
                return;
            }

            Livewire.hook("commit", ({component: i, commit: e, respond: n, succeed: t, fail: o}) => {
                t(({snapshot: l, effects: a}) => {
                    a.dispatches?.forEach(r => {
                        if (!r.params?.awaitSchemaComponent) return;
                        let s = Array.from(i.el.querySelectorAll(`[wire\\:partial="schema-component::${r.params.awaitSchemaComponent}"]`)).filter(c => u(c) === i);
                        if (s.length !== 1) {
                            if (s.length > 1) throw `Multiple schema components found with key [${r.params.awaitSchemaComponent}].`;
                            window.addEventListener(`schema-component-${i.id}-${r.params.awaitSchemaComponent}-loaded`, () => {
        ...
        // Start trying to setup the hook
        setupLivewireHook();
    });
})();
